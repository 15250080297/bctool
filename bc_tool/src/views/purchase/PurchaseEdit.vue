...
<script>
  ...
  export default {
    ...
    computed:{
      showDetail:function () {
        let val = this.$store.state.purchase.showDetail;
        this.rowData = this.$store.state.purchase.detailData.list;
        this.headerData = this.$store.state.purchase.detailData;
        return val;
      }
    },
    methods:{
          ...
          this.$confirm("确定放弃修改?", "提示", {
            ...
            callback:(action, instance)=>{
              if('confirm' == action){
                ...
                this.$store.dispatch('HidePurchaseDetail').then(()=>{});
                this.emptyList();
              }
            }
          });
