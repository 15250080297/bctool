<template>
  <div class="edit-container">
    <SCModify id="purchaseModify" :show="prShowDetail">
      <ActionBar slot="menu">
...
<script>
  ...
  import { updatePurchase,delPurchase } from '@/api/purchase'
  ...
  export default {
    ...
    computed:{
      prShowDetail:function () {
        let val = this.$store.state.purchase.prShowDetail;
        this.rowData = this.$store.state.purchase.prDetailData.list;
        this.headerData = this.$store.state.purchase.prDetailData;
        return val;
      }
    },
    methods:{
          ...
          this.$confirm("确定放弃修改?", "提示", {
            ...
            callback:(action, instance)=>{
              if('confirm' == action){
                ...
                this.$store.dispatch('HidePrOrderDetail').then(()=>{});
                this.emptyList();
              }
            }
          });
      ...
      save:function () {
        this.$refs['headerData'].validate((valid) => {
          if (valid) {
            ...
            updatePurchase(JSON.stringify(this.headerData)).then((response) => {
              if (response.code == 0) {
      ...
      del:function () {
        this.$confirm("确定删除该采购退货单?", "提示", {
          type:'info',
          callback:(action, instance)=>{
            if('confirm' == action){
              delPurchase(JSON.stringify(this.headerData)).then((response) => {
                if(response.code == 0 ){
