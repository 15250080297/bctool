<template>
  <div class="tab-container">
    ...
    <ActionBar>
      ...
      <el-button class="action-item" type="primary" @click="addSales">{{$t('button.add')}}</el-button>
    ...
    <!-- 查询 -->
    <SCSearch id="salesSearchForm" title="销售订单查询">
      <el-form :model="form">
...
<script>
  ...
  import SalesAdd from './SalesAdd.vue'
  ...
  export default {
    name: 'tab',
    components: {ActionBar,SCContent, SCTable, SCSearch, SCDateRange,SalesEdit,SalesAdd,ApprovalStream,SalesImport},
    data() {
      return {
        gridOptions:{
          defaultColDef:{
    ...
    created:function () {
      ...
      this.$root.$off('search:salesSearchForm');
      this.$root.$on('search:salesSearchForm',()=>{
        this.searchData();
      });
    ...
    methods: {
      onRowSelected: function (event) {
        ...
        findDetail(JSON.stringify({"salesOrderId": row.id})).then((response) => {
          if (response.code == 0) {
      ...
      onRowDoubleClicked: function (event) {
        ...
        couldUpdate(JSON.stringify({"salesOrderId": event.data.id})).then((response) => {
          //   console.info(response.data)
            if (!response.data) {
              this.$alert('该销售订单已被引用，无法修改', '提示', {
                confirmButtonText: '确定',
              });
            } else {
              this.$store.dispatch('ShowSalesDetail', event.data).then(() => {
              });
            }
          }
        );
      ...
      showSearchForm: function () {
        this.$root.$emit('showSearch:salesSearchForm');
      },
